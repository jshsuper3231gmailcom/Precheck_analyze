...
② 로그 타입별 분석 수행
   ...
   - 존재: 로그 자체가 에러     → 에러
   - 정보: 분석 없이 저장       → 저장만
   - 비교: 두 수치 A==B 비교    → 정상/경고/에러 (LOG_ID에 허용값(%) 설정 시 차이가 이내면 경고)
...
CREATE TABLE TB_ANALYZE_RESULT (
    ...
    -- 허용값:
    --   '정상'   : 분석 정책에 합당한 경우
    --   '경고'   : 임계치 미만이지만 근접한 경우 (수치 타입), 또는 두 수치 차이가 허용값(%) 이내인 경우 (비교 타입)
    ...
    -- 수치 타입: 임계치 근접 비율 (예: 임계치 100의 20% 이내 → WARNING_RATIO = 20.00)
    -- 비교 타입: LOG_ID에 설정된 허용값(%) (예: 허용값 5(%) → WARNING_RATIO = 5.00)
...
[비교 타입]
  ...
  경고: "[경고][{LOG_ID}] {LOG_CONTENT} (A={수치A}, B={수치B}, 불일치하나 B 대비 허용값 {RATIO}% 이내 근접)"
  에러: "[에러][{LOG_ID}] {LOG_CONTENT} (A={수치A}, B={수치B}, 불일치)"
  예)   "[에러][JUCHE_DIFF_01] 실시간정산 #1 수신$123$ 처리$120$ (A=123, B=120, 불일치)"
  예)   "[경고][JUCHE_DIFF_01] 실시간정산 #1 수신$99$ 처리$100$ (A=99, B=100, 불일치하나 B 대비 허용값 5% 이내 근접)"
  -> 경고는 LOG_ID 정책에 허용값(%)이 설정된 경우만 발생, 미설정 시 불일치는 항상 에러
